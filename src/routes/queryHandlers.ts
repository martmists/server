 ...
 */
export function handlePagination<T extends BaseEntity>(req: Request, res: Response, target: T, relations?: string[]): T[] {
    const limit = Math.min(Number(req.query.limit || 20), 100);
    const page = Number(req.query.page || 1);
    const ret: T[] = target.find({take: limit, skip: page * limit, relations});
